...
class NegativeSampling():
    def __init__(self):
        self.lr = settings['lr']
        self.negative_sample = settings['negative_sample']
        ...
        self.vocab_size = len(self.vocab_count.keys())

        self.vocab_list = sorted(list(self.vocab_count.keys()), reverse=False)
        ...
        # Unigram
        self.unigram_p = list(np.zeros(self.vocab_size))
        for i in range(self.vocab_size):
            self.unigram_p[i] = self.vocab_count[self.vocab_list[i]]

        self.unigram_p = np.power(self.unigram_p, 0.75)
        self.unigram_p /= (np.sum(self.unigram_p))
        self.unigram_p = list(self.unigram_p)
            ...
            for i, word in enumerate(sentence):
                ...
                for j in range(i-(window_size//2), i+(window_size//2)+1):
                    if j!=i and j<=sent_len-1 and j>=0:
                        positive.append(self.word_ohe(sentence[j]))
                        neg = []
                        for k in range(self.negative_sample):
                            neg.append(
                                self.word_ohe(
                                np.random.choice(self.vocab_list, replace=False,
                                                 p=self.unigram_p)))
                            if neg not in negative:
                                negative.append(neg)

                train_data.append([w_target, positive, negative])
    ...
    def forward(self, x_idx, t_idx, neg):
        loss = 0
        h = self.w1[x_idx]
        ...
        out = np.sum(emb * h)
        out = 1 / (1 + np.exp(-out))
        correct_label = np.ones_like(out)
        loss += self.cross_entropy_error(out, correct_label)

        #negative sample
        negative_output = []
        for i in range(self.negative_sample):
            neg_idx = list(neg[i]).index(1)
            neg_emb = self.w2[:, neg_idx]
            neg_out = np.sum(neg_emb * h)
            neg_out = 1 / (1 + np.exp(-neg_out))
            negative_output.append(neg_out)
            negative_label = np.zeros_like(neg_out)
            loss += self.cross_entropy_error(neg_out, negative_label)

        return loss, out, negative_output, h


    def backward(self, y_pred, neg_pred, x, h, context):
        dh = 0
        dscore = (y_pred - 1) * h
        dscore = dscore.reshape((1,100))
        self.w2[:, x.index(1)] = self.w2[:, x.index(1)] - (self.lr * dscore)
        dh += dscore
        ...
        self.w1[x.index(1)] = self.w1[x.index(1)] - (self.lr * dh)


    def train(self):
        self.w1 = np.random.uniform(-0.99, 0.99, (self.vocab_size, self.dim))
        self.w2 = np.random.uniform(-0.99, 0.99, (self.dim, self.vocab_size))


        for i in range(self.epoch):
            train_data = self.generate_train_data(corpus)
            self.loss = 0

            for j in range(len(train_data)):
                target = train_data[j][0]
                context = train_data[j][1]
                neg = train_data[j][2]

                for k in range(len(context)):
                    loss, y_pred, neg_pred, h = self.forward(list(target).index(1), list(context[k]).index(1), neg[k])

                    self.backward(y_pred, neg_pred, target, h, context[k])
...
if __name__ == '__main__':
    current = time.time()
    ns = NegativeSampling()
    a = ns.generate_train_data(corpus)
